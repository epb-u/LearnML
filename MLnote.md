...
​	梯度下降算法通过多次迭代求得使代价函数$J(\theta)$最小的$\theta$，而正规方程算法可以一次性的计算出$\theta$。

​	具体方法就是另$\frac{\partial}{\partial{\theta_{j}}}J\left( {\theta_{j}} \right)=0$, 假设我们的训练集特征矩阵为 $X$（包含了 ${{x}_{0}}=1$）并且我们的训练集结果为向量 $y$，则利用正规方程解出向量 $\theta ={{\left( {X^T}X \right)}^{-1}}{X^{T}}y$ 。

​	(具体为什么这个公式能求出$\theta$，暂时省略证明)
...
​	在之前的章节中提到了特征缩放的技巧，此处的正规方程算法不是通过迭代求出参数的，所以不需要使用特征缩放

​	对于那些不可逆的矩阵（通常是因为特征之间不独立，如同时包含英尺为单位的尺寸和米为单位的尺寸两个特征，也有可能是特征数量大于训练集的数量），正规方程方法是不能用的。

| 梯度下降                                       | 正规方程                                        |
| ---------------------------------------------- | ----------------------------------------------- |
| 缺点：需要设定学习率$\alpha$，还需要很多次迭代 | 优点：没有梯度下降的缺点                        |
| 优点：即使特征很多，也能正常运行               | 缺点：需要计算$(X^TX)^{-1}$,如果n很大就要算很久 |

# 5 Octave教程

​	现在主要使用python，因此略过这部分内容

# 6 逻辑回归

## 6.1 分类问题

​	在分类问题中要预测结果是否属于一个类(如：正确或错误)。在二分类问题中，将因变量y可能属于的两个类分别称为负向类（**negative class**）和正向类（**positive class**）。

​	如果使用**线性回归算法**解决一个分类问题，那么假设函数的输出值可能远大于1，或远小于0，这和训练样本的标签全都是0或1是矛盾的，所以最好使用**逻辑回归算法**

​	（逻辑回归算法实际上是一个分类算法，叫这个名字只是历史原因）

## 6.2 模型描述

​	在线性回归问题中，我们假设预测函数是$h_{\theta} \left( x \right)={\theta^{T}}X$，但将这个函数应用到逻辑回归问题就会出现如下的问题
...
​	但这时增加一个新的数据-一个尺寸非常大的恶性肿瘤，如下图，获得了一条新的直线，这条直线会输出

![](pic/6-2.jpg)

​	预测函数就会输出会超过1,因此继续用0.5作为阈值就不合适了。

​	因此对于分类问题引入新的模型：逻辑回归，该模型的输出始终在0到1之间，该模型的假设是：$h_\theta \left( x \right)=g\left(\theta^{T}X \right)$
...
对模型的理解：$h_\theta \left( x \right)$的作用是，对于给定的输入变量，根据选择的参数计算输出变量=1的可能性（**estimated probablity**）即$h_\theta \left( x \right)=P\left( y=1|x;\theta \right)$
